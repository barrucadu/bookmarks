...
async fn create_index(client: &Elasticsearch, args: CreateIndexArgs) {
    if args.drop_existing {
        match bookmarks::es::drop(client).await {
            Ok(_) => {
    ...
    match bookmarks::es::create(client).await {
        Ok(_) => {
...
async fn export_index(client: &Elasticsearch) {
    match bookmarks::es::export(client).await {
        Ok(records) => serde_json::to_writer(io::stdout(), &records).unwrap(),
        Err(error) => {
...
async fn import_index(client: &Elasticsearch, args: CreateIndexArgs) {
    ...
    match io::read_to_string(io::stdin()) {
        Ok(stdin) => match serde_json::from_str(&stdin) {
            Ok(records) => match bookmarks::es::import(client, records).await {
